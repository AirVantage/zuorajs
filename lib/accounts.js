...
module.exports = function (zuoraClient) {
  ...
  function updateAccountFromAccountKey(accountKey, updatedContent) {
    return zuoraClient.authenticate().then((headers) => {
      const url = zuoraClient.serverUrl + '/v1/accounts/' + accountKey;
      const query = {
        headers,
        body: updatedContent,
        json: true,
  ...
  return {
    ...
    updateFromAccountKey: (accountKey, updatedContent) => updateAccountFromAccountKey(accountKey, updatedContent),
