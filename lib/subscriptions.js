...
module.exports = (zuoraClient) => {
  function getSubscription(subscriptionNumberOrId) {
    return zuoraClient.authenticate().then((headers) => {
      const url = `${zuoraClient.serverUrl}/subscriptions/${subscriptionNumberOrId}`;
      const query = {
        headers,
        json: true,
      };
  ...
  function getSubscriptionsForAccount(accountId) {
    ...
    return zuoraClient.authenticate().then((headers) => {
      const url = `${zuoraClient.serverUrl}/subscriptions/accounts/${accountId}`;
      const query = {
        headers,
        json: true,
      };
  ...
  function createSubscription(subscriptionCreateRequest) {
    return zuoraClient.authenticate().then((headers) => {
      const url = `${zuoraClient.serverUrl}/subscriptions`;
      const query = {
  ...
  function activateSubscription(id, date) {
    return zuoraClient.authenticate().then((authCookie) => {
      const url = `${zuoraClient.serverUrl}/object/subscription/${id}`;
      const query = {
        body: {
