...
module.exports = function(zuoraClient) {
  function getInvoice(invoiceId) {
    return zuoraClient.authenticate().then(headers => {
      var url = zuoraClient.serverUrl + '/v1/object/invoice/' + invoiceId;
      var query = {
        headers,
        json: true
      };
  ...
  function createInvoice(invoiceCreateRequest) {
    return zuoraClient.authenticate().then(headers => {
      var url = zuoraClient.serverUrl + '/v1/object/invoice';
      var query = {
  ...
  function getByAccount(accountId) {
    return zuoraClient.authenticate().then(headers => {
      var url = `${zuoraClient.serverUrl}/v1/transactions/invoices/accounts/${accountId}`;
      var query = {
        headers,
        json: true
      };
