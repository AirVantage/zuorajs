...
module.exports = (zuoraClient) => {
  function getInvoice(invoiceId) {
    return zuoraClient.authenticate().then((headers) => {
      const url = `${zuoraClient.serverUrl}/object/invoice/${invoiceId}`;
      const query = {
        headers,
        json: true,
      };
  ...
  function createInvoice(invoiceCreateRequest) {
    return zuoraClient.authenticate().then((headers) => {
      const url = `${zuoraClient.serverUrl}/object/invoice`;
      const query = {
  ...
  function getByAccount(accountId) {
    return zuoraClient.authenticate().then((headers) => {
      const url = `${zuoraClient.serverUrl}/transactions/invoices/accounts/${accountId}`;
      const query = {
        headers,
        json: true,
      };
