...
module.exports = function(zuoraClient) {
  ...
  function getInvoiceFromInvoiceNumber(invoiceNumber, fields) {
    return zuoraClient.queryFirst(`select ${fields.join(',')} from Invoice where InvoiceNumber='${invoiceNumber}'`);
  }
  ...
  function getByAccount(accountId) {
    return zuoraClient.authenticate().then(headers => {
      var url = `${zuoraClient.serverUrl}/transactions/invoices/accounts/${accountId}`;
      var query = {
        headers,
        json: true
      };
  ...
  return {
    getFromInvoiceNumber: (invoiceNumber, fields = ['Id']) => getInvoiceFromInvoiceNumber(invoiceNumber, fields),
