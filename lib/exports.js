...
module.exports = (zuoraClient) => {
  function exportZOQL(zoql, format) {
    return zuoraClient.authenticate().then((headers) => {
      const url = `${zuoraClient.serverUrl}/object/export`;
      const data = {
        Query: zoql,
        Format: format,
      };
  ...
  function retrieveExport(exportId) {
    return zuoraClient.authenticate().then((headers) => {
      const url = `${zuoraClient.serverUrl}/object/export/${exportId}`;
      const query = {
        headers,
        json: true,
      };
