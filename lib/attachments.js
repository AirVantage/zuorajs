...
module.exports = (zuoraClient) => {
  function addAttachment(associatedObjectType, associatedObjectKey, description, file) {
    return zuoraClient.authenticate().then((headers) => {
      ...
      const url = `${zuoraClient.serverUrl}/attachments/`;
      const query = {
        headers,
        query: {
  ...
  function listAttachments(associatedObjectType, associatedObjectKey) {
    return zuoraClient.authenticate().then((headers) => {
      const url = `${zuoraClient.serverUrl}/attachments/${associatedObjectType}/${associatedObjectKey}`;
      const query = {
        headers,
        json: true,
      };
  ...
  return {
    ...
    delete: (attachmentId) => zuoraClient.authenticate().then((headers) => {
      const url = `${zuoraClient.serverUrl}/attachments/${attachmentId}`;
      const query = {
        headers,
      };
